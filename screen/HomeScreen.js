...
export default function HomeScreen({ navigation }) {
  ...
  useEffect(() => {
    ...
    Animated.loop(
      Animated.sequence([
        Animated.timing(checkAnim, { toValue: 1, duration: 800,  useNativeDriver: false }),
        Animated.delay(2000),
        Animated.timing(checkAnim, { toValue: 0, duration: 300,  useNativeDriver: false }),
        Animated.delay(300),
      ])
    ).start();
  }, []);
  ...
  const handleAdminLogin = () => {
    if (navigation && typeof navigation.navigate === "function") {
      ...
      alert("Navigating to Admin Login!");
    }
  };

  const handleOperatorLogin = () => {
    if (navigation && typeof navigation.navigate === "function") {
      navigation.navigate("Login", { role: "Operator" });
    } else {
      alert("Navigating to Operator Login!");
    }
  };
    ...
    <View style={styles.root}>
      ...
      <View style={styles.content}>
        ...
        <Animated.Text style={[styles.subtitle, { opacity: subtitleOpacity, transform: [{ translateY: subtitleY }] }]}>
          Find and book your bus tickets effortlessly
        </Animated.Text>

        {/* Feature cards */}
        <Animated.View style={[styles.featuresRow, { opacity: cardOpacity, transform: [{ translateY: cardY }] }]}>
          {features.map((f, i) => {
            const dotScales = [dotScale1, dotScale2, dotScale3];
            return (
        ...
        {/* Dots */}
        <Animated.View style={[styles.dotsRow, { opacity: btnOpacity }]}>
        ...
        <Animated.View style={[styles.roleRow, { opacity: roleOpacity, transform: [{ translateY: roleY }] }]}>
          ...
          <TouchableOpacity style={styles.roleCard} onPress={handleAdminLogin} activeOpacity={0.8}>
            ...
            <Text style={styles.roleSubLabel}>Login →</Text>
          </TouchableOpacity>
          ...
          {/* Bus Operator */}
          <TouchableOpacity style={styles.roleCard} onPress={handleOperatorLogin} activeOpacity={0.8}>
            <View style={[styles.roleIconBg, { backgroundColor: "#fbbf2422" }]}>
              <MaterialCommunityIcons name="bus-clock" size={20} color="#fbbf24" />
            </View>
            <Text style={styles.roleLabel}>Bus Operator</Text>
            <Text style={styles.roleSubLabel}>Login →</Text>
          </TouchableOpacity>
        ...
        <Animated.Text style={[styles.signinText, { opacity: btnOpacity }]}>
          ...
          <Text style={styles.signinLink} onPress={handleGetStarted}>Sign In</Text>
        </Animated.Text>
